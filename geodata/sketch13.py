...
goes_b3_lon0=0
goes_b3_lon1=3311+1
goes_b3_lat  = goes_b3_ds['lat'][goes_b3_lat0:goes_b3_lat1,goes_b3_lon0:goes_b3_lon1]
goes_b3_lon  = goes_b3_ds['lon'][goes_b3_lat0:goes_b3_lat1,goes_b3_lon0:goes_b3_lon1]
goes_b3_data = goes_b3_ds['data'][0,goes_b3_lat0:goes_b3_lat1,goes_b3_lon0:goes_b3_lon1]
# print('goes_b3_elemRes = ',goes_b3_ds['elemRes'])
# print('goes_b3_elemRes = ',goes_b3_ds['elemRes'].long_name)
# print('goes_b3_elemRes = ',goes_b3_ds['elemRes'][0])
# print('goes_b3_elemRes = ',goes_b3_ds['elemRes'].units)
# print('goes_b3_elemRes = ',goes_b3_ds['lineRes'])
# print('goes_b3_elemRes = ',goes_b3_ds['lineRes'][0])
# exit()
goes_b3_elemRes = goes_b3_ds['elemRes'][0]*0.5
goes_b3_lat_flat = goes_b3_lat.flatten()
goes_b3_lon_flat = goes_b3_lon.flatten()
goes_b3_data_flat = goes_b3_data.flatten()
goes_b3_indices = ps.from_latlon(goes_b3_lat_flat,goes_b3_lon_flat,int(gd.resolution(goes_b3_elemRes)))
print('goes_b3_indices size: ',goes_b3_indices.size)
print('goes_b3_indices type: ',type(goes_b3_indices.size))
print('goes_b3 resolution:   ',goes_b3_elemRes,gd.resolution(goes_b3_elemRes))
# exit()

if False:
    plt.figure()
    plt.imshow(goes_b3_data)
    plt.show()

# exit()

if cropped:
    goes_b3_crop_idx = np.where(\
                           (goes_b3_lat_flat > crop_lat[0]) & (goes_b3_lat_flat < crop_lat[1]) & \
                           (goes_b3_lon_flat > crop_lon[0]) & (goes_b3_lon_flat < crop_lon[1]) )
    goes_b3_lat_flat = goes_b3_lat_flat[goes_b3_crop_idx]
    goes_b3_lon_flat = goes_b3_lon_flat[goes_b3_crop_idx]
    goes_b3_indices = goes_b3_indices[goes_b3_crop_idx]
    goes_b3_data_flat = goes_b3_data_flat[goes_b3_crop_idx]    
    print('gb3 cropped length: ', goes_b3_data_flat.size)

if False:
    print('goes data 0,0 : ',goes_b3_data[0,0])
    nbins = 10
    plt.figure()
    n,bins,patches = plt.hist(goes_b3_data.flatten(),nbins,facecolor='blue',alpha=0.5)
    plt.show()
    exit()
...
print('goes_b3_lat.shape:    ',goes_b3_lat.shape)
print('goes_b3_lon.shape:    ',goes_b3_lon.shape)
print('goes_b3_data.shape:   ',goes_b3_data.shape)
print('goes_b3_elemRes (km): ',goes_b3_elemRes)
print('goes_b3_ res(elemRes):',gd.resolution(goes_b3_elemRes))
print('goes_b3_type(lat[0]): ',type(goes_b3_lat[0,0]))

if False:
# Reduce the ROI and check
    i0 = 500*goes_b3_lat1
    n = 10
    goes_b3_lat_flat = goes_b3_lat.flatten()[i0:i0+n]
    # print('type(lat_flat[0]): ',type(lat_flat[0]))
    goes_b3_lon_flat = goes_b3_lon.flatten()[i0:i0+n]
    goes_b3_data_flat = goes_b3_data.flatten()[i0:i0+n]
    goes_b3_indices = np.zeros([n],dtype=np.int64)
    goes_b3_indices = ps.from_latlon(goes_b3_lat_flat,goes_b3_lon_flat,int(gd.resolution(goes_b3_elemRes)))
    goes_b3_indices = ps.from_latlon(goes_b3_lat_flat,goes_b3_lon_flat,int(gd.resolution(goes_b3_elemRes)))
    if False:
        for i in range(n):
            print(i,goes_b3_lat_flat[i],goes_b3_lon_flat[i],hex(goes_b3_indices[i]))

# exit()

print('0 gb3ind type: ',type(goes_b3_indices))
print('0 gb3ind shap: ',goes_b3_indices.shape)
# goes_b3_indices=goes_b3_indices[50*3311:51*3311]
# goes_b3_indices=goes_b3_indices[100*3311:300:3311]
# goes_b3_indices=goes_b3_indices[299*3311:300*3311]
# goes_b3_indices=goes_b3_indices[1000*3311:1100*3311]
# print('1 gb3ind shap: ',goes_b3_indices.shape)
# print('1 gb3ind type: ',type(goes_b3_indices))
...
#     cmp = np.nonzero(ps.cmp_spatial([m2_ar_indices[i]],goes_b3_indices))
...
m2_g5_matches = {}
# k=0
for i in range(len(goes_b3_indices)):
    # k=k+1; print('comparison iter k ',k)
    # print('i ',i,type(goes_b3_indices[i]),type(m2_ar_indices))
    cmp = np.nonzero(ps.cmp_spatial(np.array([goes_b3_indices[i]],dtype=np.int64),m2_ar_indices))
    if cmp[0].size > 0:
        for j in cmp[0]:
...
if True:
    ...
    # plt.contourf(goes_b3_lon,goes_b3_lat,goes_b3_data,60,transform=transf)
    plt.scatter(goes_b3_lon_flat,goes_b3_lat_flat,s=0.75,c=goes_b3_data_flat,transform=ccrs.Geodetic())
    
    # Plot m2 AR thresh.
    if False:
        # gd.plot_indices(goes_b3_indices,c='g',transform=transf)
        gd.plot_indices(m2_ar_indices[m2_ar_n0:m2_ar_n1],c='c',transform=transf,lw=0.25)
        # Plot matching goes_b3.
        for j in m2_g5_matches:
            gd.plot_indices(goes_b3_indices[m2_g5_matches[j]],c='r',transform=transf,lw=0.25)
...
if True:
    ...
    if False:
        for j in m2_g5_matches:
            x = m2_ar_data[j]
            for y in goes_b3_data_flat[m2_g5_matches[j]]:
    ...
    if True:
        ...
        for j in m2_g5_matches:
            x = m2_ar_data[j]
            # y = 
            ymn = np.amin(goes_b3_data_flat[m2_g5_matches[j]])
            ymx = np.amax(goes_b3_data_flat[m2_g5_matches[j]])
            ystd = np.std(goes_b3_data_flat[m2_g5_matches[j]])
            ybar = np.mean(goes_b3_data_flat[m2_g5_matches[j]])
            xdat[k] = x
            ydat[k] = ybar
            wght[k] = 1.0/ystd
            k=k+1
...
# For m2 vs. b3
## run_name:  b3
## coeffs:  [ -140.67773728 11148.66902229]
## 1 threshold m2=20:  8335.11427674312
...
## >>> p(20)
## So the threshold for b3 is < ~8300
...
plt.scatter(goes_b3_lon_flat,goes_b3_lat_flat,s=30,c=goes_b3_data_flat,transform=ccrs.Geodetic())
gd.plot_indices(goes_b3_indices,transform=ccrs.Geodetic())

if True:
    print('Add circles around observation points.')
    for i in range(goes_b3_lon_flat.size):
        lonplot,latplot = gd.bbox_lonlat(goes_b3_lat_flat[i],goes_b3_lon_flat[i],goes_b3_elemRes,close=True)
        ax.plot(lonplot,latplot,True,transform=ccrs.Geodetic())
        ax.add_patch(plt.Circle((goes_b3_lon_flat[i],goes_b3_lat_flat[i]),radius=goes_b3_elemRes*180.0/(np.pi*6371.0),fill=False,transform=ccrs.Geodetic()))

if False:
    i = 2;
    print('Add a "bounding box" around point ',i)
    lonplot,latplot = gd.bbox_lonlat(goes_b3_lat_flat[i],goes_b3_lon_flat[i],goes_b3_elemRes,close=False)
    ...
    print('cmp len goes_b3_indices: ',len(goes_b3_indices),goes_b3_indices.shape)
    ...
    for i in range(ngoes_b3_indices):
        for j in range(nhull):
            if((cmp[i*nhull+j] != 0 or cmpr[j*ngoes_b3_indices+i]) != 0):
                ping = "***"
            else:
                ping = ""
            print(i,j,' i,j,cmp: ',(cmp[i*nhull+j],cmpr[j*ngoes_b3_indices+i]),hex(goes_b3_indices[i]),hex(hull[j]))
