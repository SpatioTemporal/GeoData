...
goes_b4_lon0=0
goes_b4_lon1=3311+1
goes_b4_lat  = goes_b4_ds['lat'][goes_b4_lat0:goes_b4_lat1,goes_b4_lon0:goes_b4_lon1]
goes_b4_lon  = goes_b4_ds['lon'][goes_b4_lat0:goes_b4_lat1,goes_b4_lon0:goes_b4_lon1]
goes_b4_data = goes_b4_ds['data'][0,goes_b4_lat0:goes_b4_lat1,goes_b4_lon0:goes_b4_lon1]
# print('goes_b4_elemRes = ',goes_b4_ds['elemRes'])
# print('goes_b4_elemRes = ',goes_b4_ds['elemRes'].long_name)
# print('goes_b4_elemRes = ',goes_b4_ds['elemRes'][0])
# print('goes_b4_elemRes = ',goes_b4_ds['elemRes'].units)
# print('goes_b4_elemRes = ',goes_b4_ds['lineRes'])
# print('goes_b4_elemRes = ',goes_b4_ds['lineRes'][0])
# exit()
goes_b4_elemRes = goes_b4_ds['elemRes'][0]*0.5
goes_b4_lat_flat = goes_b4_lat.flatten()
goes_b4_lon_flat = goes_b4_lon.flatten()
goes_b4_data_flat = goes_b4_data.flatten()
goes_b4_indices = ps.from_latlon(goes_b4_lat_flat,goes_b4_lon_flat,int(gd.resolution(goes_b4_elemRes)))
print('goes_b4_indices size: ',goes_b4_indices.size)
print('goes_b4_indices type: ',type(goes_b4_indices.size))
print('goes_b4 resolution:   ',goes_b4_elemRes,gd.resolution(goes_b4_elemRes))
# exit()

if False:
    plt.figure()
    plt.imshow(goes_b4_data)
    plt.show()

# exit()

if cropped:
    goes_b4_crop_idx = np.where(\
                           (goes_b4_lat_flat > crop_lat[0]) & (goes_b4_lat_flat < crop_lat[1]) & \
                           (goes_b4_lon_flat > crop_lon[0]) & (goes_b4_lon_flat < crop_lon[1]) )
    goes_b4_lat_flat = goes_b4_lat_flat[goes_b4_crop_idx]
    goes_b4_lon_flat = goes_b4_lon_flat[goes_b4_crop_idx]
    goes_b4_indices = goes_b4_indices[goes_b4_crop_idx]
    goes_b4_data_flat = goes_b4_data_flat[goes_b4_crop_idx]    
    print('gb4 cropped length: ', goes_b4_data_flat.size)

if False:
    print('goes data 0,0 : ',goes_b4_data[0,0])
    nbins = 10
    plt.figure()
    n,bins,patches = plt.hist(goes_b4_data.flatten(),nbins,facecolor='blue',alpha=0.5)
    plt.show()
    exit()
...
print('goes_b4_lat.shape:    ',goes_b4_lat.shape)
print('goes_b4_lon.shape:    ',goes_b4_lon.shape)
print('goes_b4_data.shape:   ',goes_b4_data.shape)
print('goes_b4_elemRes (km): ',goes_b4_elemRes)
print('goes_b4_ res(elemRes):',gd.resolution(goes_b4_elemRes))
print('goes_b4_type(lat[0]): ',type(goes_b4_lat[0,0]))

if False:
# Reduce the ROI and check
    i0 = 500*goes_b4_lat1
    n = 10
    goes_b4_lat_flat = goes_b4_lat.flatten()[i0:i0+n]
    # print('type(lat_flat[0]): ',type(lat_flat[0]))
    goes_b4_lon_flat = goes_b4_lon.flatten()[i0:i0+n]
    goes_b4_data_flat = goes_b4_data.flatten()[i0:i0+n]
    goes_b4_indices = np.zeros([n],dtype=np.int64)
    goes_b4_indices = ps.from_latlon(goes_b4_lat_flat,goes_b4_lon_flat,int(gd.resolution(goes_b4_elemRes)))
    goes_b4_indices = ps.from_latlon(goes_b4_lat_flat,goes_b4_lon_flat,int(gd.resolution(goes_b4_elemRes)))
    if False:
        for i in range(n):
            print(i,goes_b4_lat_flat[i],goes_b4_lon_flat[i],hex(goes_b4_indices[i]))

# exit()

print('0 gb4ind type: ',type(goes_b4_indices))
print('0 gb4ind shap: ',goes_b4_indices.shape)
# goes_b4_indices=goes_b4_indices[50*3311:51*3311]
# goes_b4_indices=goes_b4_indices[100*3311:300:3311]
# goes_b4_indices=goes_b4_indices[299*3311:300*3311]
# goes_b4_indices=goes_b4_indices[1000*3311:1100*3311]
# print('1 gb4ind shap: ',goes_b4_indices.shape)
# print('1 gb4ind type: ',type(goes_b4_indices))
...
#     cmp = np.nonzero(ps.cmp_spatial([m2_ar_indices[i]],goes_b4_indices))
...
m2_g5_matches = {}
# k=0
for i in range(len(goes_b4_indices)):
    # k=k+1; print('comparison iter k ',k)
    # print('i ',i,type(goes_b4_indices[i]),type(m2_ar_indices))
    cmp = np.nonzero(ps.cmp_spatial(np.array([goes_b4_indices[i]],dtype=np.int64),m2_ar_indices))
    if cmp[0].size > 0:
        for j in cmp[0]:
...
if True:
    ...
    # plt.contourf(goes_b4_lon,goes_b4_lat,goes_b4_data,60,transform=transf)
    plt.scatter(goes_b4_lon_flat,goes_b4_lat_flat,s=0.75,c=goes_b4_data_flat,transform=ccrs.Geodetic())
    
    # Plot m2 AR thresh.
    if False:
        # gd.plot_indices(goes_b4_indices,c='g',transform=transf)
        gd.plot_indices(m2_ar_indices[m2_ar_n0:m2_ar_n1],c='c',transform=transf,lw=0.25)
        # Plot matching goes_b4.
        for j in m2_g5_matches:
            gd.plot_indices(goes_b4_indices[m2_g5_matches[j]],c='r',transform=transf,lw=0.25)
...
if True:
    ...
    if False:
        for j in m2_g5_matches:
            x = m2_ar_data[j]
            for y in goes_b4_data_flat[m2_g5_matches[j]]:
    ...
    if True:
        ...
        for j in m2_g5_matches:
            x = m2_ar_data[j]
            # y = 
            ymn = np.amin(goes_b4_data_flat[m2_g5_matches[j]])
            ymx = np.amax(goes_b4_data_flat[m2_g5_matches[j]])
            ystd = np.std(goes_b4_data_flat[m2_g5_matches[j]])
            ybar = np.mean(goes_b4_data_flat[m2_g5_matches[j]])
            xdat[k] = x
            ydat[k] = ybar
            wght[k] = 1.0/ystd
            k=k+1
        ...
        print('coeffs: ',coeffs)
        # print('0 threshold m2=20: ',p(20,coeffs))
        poly   = np.poly1d(coeffs)
        print('1 threshold m2=20: ',poly(20))
        if plot_regression:
...
# For m2 vs. b4
# run_name:  b4
# coeffs:  [ -276.0362091 18545.0686353]
# 1 threshold m2=20:  13024.34445336238
#
## def p(x,c):
##    return x*c[0]+c[1]
## >>> def p(x):
...
## >>> p(20)
## So the threshold for b4 is < ~8300
...
plt.scatter(goes_b4_lon_flat,goes_b4_lat_flat,s=30,c=goes_b4_data_flat,transform=ccrs.Geodetic())
gd.plot_indices(goes_b4_indices,transform=ccrs.Geodetic())

if True:
    print('Add circles around observation points.')
    for i in range(goes_b4_lon_flat.size):
        lonplot,latplot = gd.bbox_lonlat(goes_b4_lat_flat[i],goes_b4_lon_flat[i],goes_b4_elemRes,close=True)
        ax.plot(lonplot,latplot,True,transform=ccrs.Geodetic())
        ax.add_patch(plt.Circle((goes_b4_lon_flat[i],goes_b4_lat_flat[i]),radius=goes_b4_elemRes*180.0/(np.pi*6371.0),fill=False,transform=ccrs.Geodetic()))

if False:
    i = 2;
    print('Add a "bounding box" around point ',i)
    lonplot,latplot = gd.bbox_lonlat(goes_b4_lat_flat[i],goes_b4_lon_flat[i],goes_b4_elemRes,close=False)
    ...
    print('Compare the goes_b4_indices with the "hulled bounding box."')
    print('cmp len goes_b4_indices: ',len(goes_b4_indices),goes_b4_indices.shape)
    print('cmp len hull:    ',len(hull),hull.shape)
    cmp = ps.cmp_spatial(goes_b4_indices,hull)
    cmpr = ps.cmp_spatial(hull,goes_b4_indices)
    ngoes_b4_indices = len(goes_b4_indices)
    nhull = len(hull)
    for i in range(ngoes_b4_indices):
        for j in range(nhull):
            if((cmp[i*nhull+j] != 0 or cmpr[j*ngoes_b4_indices+i]) != 0):
                ping = "***"
            else:
                ping = ""
            print(i,j,' i,j,cmp: ',(cmp[i*nhull+j],cmpr[j*ngoes_b4_indices+i]),hex(goes_b4_indices[i]),hex(hull[j]))
