...
# [np.where(b45_img>1000)]

b45_img_tot = b45_img

# m2_img_ge2_idx = np.where((m2_img >= 20.0)) # This is where TPW is high and b5 is low.
# m2_img_lt2_idx = np.where((m2_img < 20.0))  # Reverse.

m2_img_ge2_idx = np.where((m2_img >= 20.0) & (b45_img < -250)) # This is where TPW is high and b5 is low.
m2_img_lt2_idx = np.where((m2_img < 20.0)  & (b45_img < -250))  # Reverse.
...
if False:
    ...
    for i in group:    
        ...
        ax.scatter(x0,ybar[k],s=30,c='darkred')
        ax.scatter([x0,x0],[ymn[k],ymx[k]],s=15,c='coral')
        ax.plot([x0,x0],[ymn[k],ymx[k]],alpha=0.5,c='red')
        k=k+1

if False:
    ax.plot(xdat,poly(xdat),c='black')
    # print('annotating')
    # ax.annotate('y = %4.1f x + %5.0f'%(coeffs[0],coeffs[1]),xy=(8,-750))
    # ax.annotate("Testing1",xy=(0.25,0.7),xycoords='figure fraction')
    # ax.annotate("Testing2",xy=(8,-750),xycoords='data')
    ax.text(11,-750,'y = %4.1f x + %5.0f'%(coeffs[0],coeffs[1]),horizontalalignment='left')
    ax.text(11,-900,'y_thr = %5.0f'%poly(20.0),horizontalalignment='left')
    ax.set_title("Comparing B4-B5 with M2.TPW")
    ax.text(8,-2750,workFileName)
    plt.show()
    # exit()
...
ax1.set_title('GOES B4-B5 decomposed against M2-TPW>2')
# ax1.annotate("b5[m2>2]/tot.",xy=(2000,500000))
# ax1.annotate("b5[m2<2]/tot.", xy=(2000,250000))

ax1.set_xlabel('B4-B5 Raw Counts')
ax1.set_ylabel('Histogram')

alpha=0.6
n,bins_tot,patches_tot = ax1.hist(b45_img_tot,bins=bins_,facecolor='blue',alpha=alpha)
n,bins_ge,patches_ge   = ax1.hist(b45_img_ge,bins=bins_,facecolor='cyan',alpha=alpha)
n,bins_lt,patches_lt   = ax1.hist(b45_img_lt,bins=bins_,facecolor='red',alpha=alpha)

y_hist_tot = [ i.get_height() for i in patches_tot ]
y_hist_ge  = [ i.get_height() for i in patches_ge ]
y_hist_lt  = [ i.get_height() for i in patches_lt ]

print('y_hist_tot mnmx: ',np.amin(y_hist_tot),np.amax(y_hist_tot))
print('y_hist_ge mnmx:  ',np.amin(y_hist_ge),np.amax(y_hist_ge))
print('y_hist_lt mnmx:  ',np.amin(y_hist_lt),np.amax(y_hist_lt))
...
print('calculate decomposition and odds ratio')

bkg = 0
# bkg = 20
# bkg = 10000
# bkg = np.amin(y_hist_tot)
# bkg = np.sqrt(y_hist_tot)
...
ax2.plot([x[0],x[-1]],[1,1],color='grey')
ax2.set_ylabel('b5-b4 m2on/off ratio')

x = np.linspace(-2500,-500,100)

# mu_ge,std_ge = norm.fit(b45_img_ge)
# p_ge = norm.pdf(x,mu_ge,std_ge)

# a,loc,scale = skewnorm.fit(b45_img_ge[np.where(b45_img_ge>-1842)])
# a,loc,scale = skewnorm.fit(b45_img_ge[np.where(b45_img_ge>-2250)])
a,loc,scale = skewnorm.fit(b45_img_ge)
p_ge = skewnorm.pdf(x,a,loc,scale)
a_ge = a; loc_ge = loc; scale_ge = scale
p_ge0 = p_ge.copy()
p_ge = p_ge/np.amax(p_ge)
p_ge = p_ge*(np.amax(y_hist_ge)-bkg)
ax1.plot(x,p_ge+bkg,linewidth=2,color='cyan')
print('ge: a,loc,scale: ',a,loc,scale)

# mu_lt,std_lt = norm.fit(b45_img_lt)
# p_lt = norm.pdf(x,mu_lt,std_lt)

# a,loc,scale = skewnorm.fit(b45_img_lt[np.where((b45_img_lt<-1842))])
# a,loc,scale = skewnorm.fit(b45_img_lt[np.where((b45_img_lt<-1842))])
a,loc,scale = skewnorm.fit(b45_img_lt)
...
print('lt: a,loc,scale: ',a,loc,scale)

ax1.plot(x,p_lt+p_ge+2*bkg,linewidth=2,color='lime')

# ratio
y_ratio = (p_lt0)/(p_ge0)
ax2.plot(x,y_ratio,linewidth=3,color='orange')
print('check: xy=',(x[-2],y_ratio[-2]))
ax2.annotate('Ratio'
             ,xy=(x[-2],y_ratio[-2])
             ,xycoords='data'
             ,xytext=(x[-8],1.5),textcoords='data'
             ,arrowprops=dict(arrowstyle="->",connectionstyle="arc3",facecolor='black'))

root_result = root_scalar(lambda x: 1-(skewnorm.pdf(x,a,loc,scale)/skewnorm.pdf(x,a_ge,loc_ge,scale_ge)),x0=-1800,x1=-1850)
if root_result.converged:
    b45_thresh = root_result.root
    ax2.plot([b45_thresh,b45_thresh],ax2.get_ylim(),color='grey')
    ax2.text(b45_thresh+25,0.5,'x=%5.0f'%b45_thresh)
else:
    print("warning: root didn't converge")

plt.show()


### TODO Add page for just PDF's and the ratio.
