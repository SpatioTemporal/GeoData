...
def main():
    ...
    for i_ in i:
        for j_ in j:
            # print('ij: ',i_,j_)
            idx_ijg[j_,i_] = id_from(i_,j_,sh)
    ...
    for k in np.arange(idx_ijg.size):
        i_,j_ = from_id(idx_ijg_flat[k],sh)
        idx_ig[k] = i_
        idx_jg[k] = j_
    ...
    for i_ in i:
        for j_ in j:
            # print('ij: ',i_,j_)
            idx1_ijg[j_,i_] = id1_from(i_,j_,sh)
    ...
    for k in range(idx1_ijg_flat.size):
        ...
        i_,j_ = from_id1(idx1_ijg_flat[k])
        print(k,'i_,j_',i_,j_)
        idx1_ig_inv[k] = i_
        idx1_jg_inv[k] = j_
...
if __name__ == '__main__':
    main()
# >>>>>>> d88945326d12043da34b5d69e6f0be70be940b0d
