...
def main():
    ...
    m_dat_mnmx = [np.amin(m_dat),np.amax(m_dat)]
    vmin = m_dat_mnmx[0]
    vmax = m_dat_mnmx[1]
        ...
        if len(da_r) > 4:
            ...
            if viz and abs(idlo_skew) < 0.5 and abs(idla_skew) < 0.5:
                ...
                if True:
                    ...
                    plot_box(ax,xlim,ylim,c='orange')
                    if False:
                        ax.set_xlim(xlim[0],xlim[1])
                        ax.set_ylim(ylim[0],ylim[1])
                    
                # Plot granule outline
                plot_box(ax,mlon_mnmx,mlat_mnmx)

                # plot ROI outline (GOES search area)
                plot_box(ax,lo_range,la_range)

                # plot MODIS boxes at nadir resolution
                ...
                if True: # Plot scan lines (rows)
                    # for irow in range(2030-10,2030):
                    # for irow in range(2030):
                    # decimate = 20
                    decimate = 1
                    for irow in range(0,2030,decimate):
                        i0 = irow*1354
                        i1 = (irow+1)*1354
                        idx = range(i0,i1,decimate)
                        if False:
                            for j in range(i0,i1):
                                xr,yr=make_box(m_lon[j],m_lat[j],delta=mod_nadir_hdelta) # NADIR resolution for MODIS
                                plot_box(ax,xr,yr,c='k')
                        # ax.scatter(m_lon[idx],m_lat[idx],s=0.5,c='k',transform=transf
                        ax.scatter(m_lon[idx],m_lat[idx],c=m_dat[idx],s=0.5,transform=transf
                                   ,vmin=vmin
                                   ,vmax=vmax
                                   ,cmap='rainbow'
                                   # ,cmap='Reds'
                        ...
                        # ax.scatter(m_lon[i0:i1],m_lat[i0:i1],s=0.5,c='k',transform=transf)
                        if False:
                            ind  = m_str[i0:i1]
                ...
                if True:
                    scale = 2
                    xlim=[lo0-scale*delta,lo0+scale*delta]
                    ylim=[la0-scale*delta,la0+scale*delta]
                    plot_box(ax,xlim,ylim,c='c')                        
                ...
                if True:
                    for irow in range(2030-20,2030):
                        i0 = irow*1354
                        # i1 = (irow+1)*1354
                        i1 = i0 + 20
                        if True:
